...
declare -A KDUMP_COMMANDLINE_APPEND=(
	[default]="irqpoll maxcpus=1 reset_devices novmcoredd cma=0 hugetlb_cma=0"
...
arch="$1"
known_arches="aarch64 i386  ppc64 ppc64le s390x x86_64"
if [[ -z $arch || " ${known_arches} " != *" ${arch} "* ]]; then
	echo "Warning: Unknown architecture '$arch', using default sysconfig template." >&2
	arch="default"
fi

KEXEC_ARGS_val="${KEXEC_ARGS[$arch]:-${KEXEC_ARGS[default]}}"
KDUMP_COMMANDLINE_REMOVE_val="${KDUMP_COMMANDLINE_REMOVE[$arch]:-${KDUMP_COMMANDLINE_REMOVE[default]}}"
KDUMP_COMMANDLINE_APPEND_val="${KDUMP_COMMANDLINE_APPEND[$arch]:-${KDUMP_COMMANDLINE_APPEND[default]}}"
FADUMP_COMMANDLINE_APPEND_val="${FADUMP_COMMANDLINE_APPEND[$arch]:-${FADUMP_COMMANDLINE_APPEND[default]}}"
